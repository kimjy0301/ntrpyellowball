        ...
        courts.map((value) => {
          ...
          tmpGeocoder.addressSearch(value.location, function (result, status) {
            ...
            if (status === kakao.maps.services.Status.OK) {
              ...
              var iwContent = `<div class="p-4 rounded h-full w-60 md:text-sm">코트명 : ${value.name} <br>주소 : ${value.location} <br>전화번호 : ${value.call} <br>코트종류 : ${value.surface} <br>코트면수 : ${value.count} <br>예약가능여부 : ${value.reservation} <br> <a href="https://map.kakao.com/link/to/${value.name},${lat},${lng}" target="_blank" class="text-white bg-blue-600 rounded px-2 ">길찾기</a></div>`,
